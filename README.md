...
2. [Доступ владельца (логин/пароль)](#2-доступ-владельца)
...
В текущей сборке вход в «Кабинет» проверяется на клиенте:
...
> ⚠️ **Это прототипное поведение.** Для боевой работы перенесите проверку на backend (раздел [«Безопасность»](#10-безопасность)):
> 1. В таблицу `users` закладывается **bcrypt-хэш** пароля (никогда не открытый текст и не MD5).
> 2. Эндпоинт `POST /api/auth/login` выдаёт **JWT** (access 15 мин + refresh 30 дней, httpOnly-cookie).
> 3. Rate-limit: 5 неудач → блокировка IP на 15 минут.
> 4. В коде фронтенда строки `OWNER_LOGIN` / `OWNER_PASS` в файле `src/sections/Cabinet.tsx` **удаляются**, проверка заменяется на вызов `/api/auth/login`.

Смена пароля: обновите хэш в `users` и значение в `Cabinet.tsx` (в прототипе) либо только хэш (в production).
