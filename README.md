...
> 2. Эндпоинт `POST /api/auth/login` выдаёт **JWT с ролью** (access 15 мин + refresh 30 дней, httpOnly-cookie).
> 3. Rate-limit: 5 неудач → блокировка IP на 15 минут.
> 4. В коде фронтенда константы `OWNER_LOGIN` / `OWNER_PASS` / `USER_LOGIN` / `USER_PASS` в `src/store.tsx` **удаляются**, `loginAs` заменяется на вызов `/api/auth/login`, а доступ к мастер-разделам проверяется по `role` из JWT и на backend.

Смена пароля: обновите хэш в `users` и значение в `src/store.tsx` (в прототипе) либо только хэш (в production).
