# CONTEXT.md — текущее состояние проекта (обновляется агентом после каждой задачи)
...
НЕТ активной задачи. Сделано в сессии: cron_downgrade.php, вебхук v2, README 0.2.6, .env.example, чистка api/ и git-индекса, тарифная линейка сокращена до Free + Pro (Studio/trial убраны: payments.php, вебхук, api.ts, store.tsx, data.ts, Welcome, Cabinet, README, оферта; typecheck чист). ЗАДАЧИ ДЕПЛОЯ (на пользователе): (1) cron_downgrade.php → сервер → --dry → Beget cron; (2) yookassa.php v2 → public_html/api/webhooks/ (+YOOKASSA_SHOPID/SECRET в .env сервера; тест-платёж → RECV→SUBSCRIPTION_ACTIVATED в логе); (3) УДАЛИТЬ С СЕРВЕРА public_html/api/: debug_payments.php, fix_subscription.php, migrate_add_user_id.php, test.php — fix_subscription выдаёт pro по GET-запросу!; (4) README.md выгрузить/закоммитить; (5) config.php (CORS-whitelist) + payments.php (только pro) → public_html/api/, после деплоя проверить работу сайта и localhost:3000 у разработчика; (6) сборка фронта (убраны Studio/trial) → dist → public_html/, вместе с новой офертой. Следующий шаг: QUEUE #5 — секция Funnel (последняя крупная фича в очереди) либо деплой-батч всех накопленных файлов.
...
5. секция Funnel: таблица + эндпоинт + AI-генерация (по аналогии с product)
