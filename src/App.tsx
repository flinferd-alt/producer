...
function AppInner() {
  ...
  const launch = LAUNCHES[launchIdx];
  ...
  return (
    <div className="relative min-h-screen text-ink">
      ...
      {/* -------- sidebar -------- */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-[252px] flex-col border-r border-line bg-deep2/90 backdrop-blur-xl transition-transform duration-500 lg:translate-x-0 ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <header className="sticky top-0 z-20 border-b border-line bg-deep/85 backdrop-blur-xl">
          <div className="mx-auto flex max-w-[1440px] items-center gap-4 px-4 py-3.5 sm:px-6 lg:px-8">
            <button onClick={() => setMenuOpen(true)} className="cursor-pointer rounded-lg border border-line p-2 text-mut hover:text-ink lg:hidden" aria-label="Меню">
              <svg width="18" height="18" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" fill="none">
            ...
            <div className="ml-auto flex items-center gap-3">
              ...
              <div className="relative" ref={dropRef}>
                <button
                  ...
                  className="flex cursor-pointer items-center gap-2.5 rounded-lg border border-line bg-panel px-3 py-2 transition-colors hover:border-line2"
                >
                  ...
                  <div className="absolute right-0 top-full z-30 mt-2 w-72 overflow-hidden rounded-xl border border-line bg-panel shadow-2xl shadow-black/50">
                    {LAUNCHES.map((l, i) => (
                      <button
                        ...
                        className="flex w-full cursor-pointer items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-panel2"
                      >
                        <Dot tone={l.tone} />
                    ...
                    <div className="border-t border-line px-4 py-2.5">
                      <button onClick={() => { setLaunchOpen(false); push("Оркестратор создал сценарий нового запуска — начните с распаковки", "amber"); go("unpack"); }} className="flex w-full cursor-pointer items-center gap-2 font-mono text-[11px] tracking-wide text-amber uppercase hover:brightness-125">
                        <Icon name="spark" size={14} /> Новый запуск
                      </button>
                    </div>
                  </div>
                )}
            ...
            <div className="ticker-track flex w-max items-center gap-8 py-1.5 pl-8">
              {[...TICKER, ...TICKER].map((t, i) => (
                <span key={i} className="flex items-center gap-2.5 font-mono text-[11px] whitespace-nowrap text-mut">
                  <span className={i % 3 === 0 ? "text-amber" : i % 3 === 1 ? "text-mint" : "text-sky"}>▸</span>
                  {t}
                </span>
              ))}
            </div>
